...
int main(int argc, char *argv[]) {
  ...
  **/
  ...
  if(rank == MAITRE){
    ...
    /* Traitement du premier bloc point par point */
    // y = ymin + h_local*rank*yinc; // rank = 0 car master donc nous pouvons juste écrire
    y = ymin;
    for (i = 0; i < h_local; i++) { 
      x = xmin;
      for (j = 0; j < w; j++) {
    ...
    pima_loc = ima_loc = (unsigned char *)malloc( w*h_local*sizeof(unsigned char));
    if( ima_loc == NULL) {
      fprintf( stderr, "Erreur allocation mémoire du tableau \n");
      return 0;
    }
